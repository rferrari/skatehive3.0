...
function generatePodiumImageUrl(top3: SkaterData[]): string {
  ...
  const avatarUrls = top3.map(skater => 
    `https://images.hive.blog/u/${skater.hive_author}/avatar/small`
  ).join(',');
  
  // This would be your podium image generation endpoint
  return `${baseUrl}/api/generate-podium?avatars=${encodeURIComponent(avatarUrls)}&names=${top3.map(s => s.hive_author).join(',')}`;
}
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const dynamicDescription = top3.length > 0 
  ...
  return {
    title: `Skatehive Leaderboard | Top Skaters - ${top3[0]?.hive_author || 'Champions'} Leading`,
    ...
    openGraph: {
      title: `🏆 Skatehive Leaderboard - ${top3[0]?.hive_author || 'Champions'} on Top!`,
      description: dynamicDescription,
      type: "website",
      images: [
        {
    ...
    twitter: {
      ...
      title: `🏆 Skatehive Leaderboard - ${top3[0]?.hive_author || 'Champions'} Leading!`,
      description: dynamicDescription,
