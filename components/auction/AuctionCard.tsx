...
export default function AuctionCard({ 
  ...
  const { data: activeAuction, refetch, isLoading } = useLastAuction(tokenAddress, defaultAuction);
    ...
    >
      <VStack spacing={6} align="stretch">
        ...
        <VStack spacing={4}>
            ...
            {auctionData.isRunning && (
              <Countdown
                ...
                renderer={({ days, hours, minutes, seconds, completed }) => {
                  if (completed) {
                    return <Text color="error" fontFamily="mono">ENDED</Text>;
                  }
          ...
          {/* Bid Count */}
          <HStack justify="space-between" w="full">
            <Text fontSize="sm" color="text">Total bids: {activeAuction.bidCount}</Text>
            <Text fontSize="sm" color="text">
              Reserve: {formatBidAmount(BigInt(activeAuction.dao.auctionConfig.reservePrice))} ETH
            </Text>
          </HStack>
        </VStack>
        ...
        {activeAuction.bids && activeAuction.bids.length > 0 && (
          <>
            ...
            <VStack align="stretch" spacing={3}>
              <Text fontWeight="medium" color="text">Recent Bids</Text>
              <VStack 
                spacing={2} 
                maxH="160px" 
                overflowY="auto"
                  ...
                  .map((bid, index) => (
                    ...
                    >
                      ...
                      <Text fontSize="xs" color="muted">
                        {new Date(parseInt(bid.bidTime) * 1000).toLocaleTimeString()}
                      </Text>
                    </HStack>
                  ))}
              </VStack>
