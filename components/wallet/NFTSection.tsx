  ...
  const filteredNFTs = useMemo(() => {
    ...
    nfts = nfts.filter((nft: any) => {
      ...
      const collectionAddress =
        nft.token?.collection?.address?.toLowerCase() || "";
    ...
    if (hideFloorless) {
      nfts = nfts.filter((nft: any) => {
        const floorPrice = parseFloat(
          nft.token?.collection?.floorPriceEth || "0"
        );
        return floorPrice > 0;
      });
    }
                    ...
                    <VStack spacing={0} align="stretch">
                      {filteredNFTs.length > 0 ? (
                        ...
                        <Box py={8} textAlign="center">
                          <Text color="muted">
                            {hideFloorless
                              ? "No Gnars or SkateHive NFTs with floor price found"
                              : "No Gnars or SkateHive NFTs found in portfolio"}
                          </Text>
                        </Box>
                      )}
                    </VStack>
                          ...
                          {filteredNFTs.length > 0 ? (
                            filteredNFTs.map((nft: any, index: number) => {
                              ...
                              const estimatedValue =
                                nft.token?.estimatedValueEth || "0";
                              const rarityRank = nft.rarityRank;
                              const collectionAddress =
                                nft.token?.collection?.address;
                              const network =
                                nft.token?.collection?.network || "ethereum";
