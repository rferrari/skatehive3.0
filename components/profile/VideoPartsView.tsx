...
const VideoPartsView = memo(function VideoPartsView({
  ...
  const updateVideoParts = useCallback(async (newVideoParts: VideoPart[]) => {
    try {
      await updateProfile(
        username,
        profileData.name,
  ...
  }, [username, profileData.name, profileData.about, profileData.location, profileData.coverImage, profileData.profileImage, profileData.website, profileData.ethereum_address, toast, onProfileUpdate]);

  const handleNewVideoPart = useCallback(async (videoPart: VideoPart) => {
    const newVideoParts = [
  ...
  const handleRemoveVideoPart = useCallback(async (index: number) => {
    const newVideoParts = (
      (profileData.video_parts as unknown as VideoPart[]) || []
    ).filter((_, i) => i !== index);
    await updateVideoParts(newVideoParts);
